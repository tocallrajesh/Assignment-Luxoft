...
const Login = () => {
  ...
  const onSubmit = (data, e) => {
    ...
    fetch(`${config.baseUrl}/login`, {
      ...
      .then((data) => {      
        setMessage({
          data: data.error || "Logged in successfully, redirecting...",
          type: data.error ? "alert-danger" : "alert-success",
        });

        !data.error &&
          setTimeout(() => {
            localStorage.setItem("token", JSON.stringify({id:data[0].id,name:data[0].name,role:data[0].role}));
            history.push("/dashboard");
          }, 3000);

        !data.error && e.target.reset();
      });
  };
